...
namespace Modules\GeomapMonzphere\Actions;
...
class WidgetView extends CControllerDashboardWidgetView {
	...
	private function getHosts(): array {
		if ($this->isTemplateDashboard()) {
			if ($this->fields_values['groupids']) {
				$filter_groupids = getSubGroups($this->fields_values['groupids']);
				...
				// Armazena o groupid para ser usado na navegação
				foreach ($hosts as &$host) {
					$host['groupid'] = $this->fields_values['groupids'][0]; // Usa o primeiro grupo
				}
				unset($host);
			}
			elseif ($this->fields_values['override_hostid']) {
				$hosts = API::Host()->get([
					'output' => ['hostid', 'name'],
					'selectInventory' => ['location_lat', 'location_lon'],
	...
	private static function convertToRFC7946(array $hosts) : array {
		...
		foreach ($hosts as $host) {
			...
			// Adiciona groupid se estiver disponível
			if (isset($host['groupid'])) {
				$properties['groupid'] = $host['groupid'];
			}
