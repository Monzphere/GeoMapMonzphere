...
class CWidgetGeoMapMonzphere extends CWidget {
	...
	#broadcast() {
		const host = this.#geomap?.hosts.find(h => h.properties.hostid === this.#selected_hostid);
		let data = {
			[CWidgetsData.DATA_TYPE_HOST_ID]: [this.#selected_hostid],
			[CWidgetsData.DATA_TYPE_HOST_IDS]: [this.#selected_hostid]
		};
		
		// Não podemos fazer broadcast do groupid pois não está declarado no manifesto
		// Armazenamos como propriedade mas não enviamos no broadcast
	...
	_initMap(config) {
		...
		this._map.getContainer().addEventListener('cluster.click', (e) => {
			...
			hintbox.classList.add(CWidgetGeoMapMonzphere.ZBX_STYLE_HINTBOX);
	...
	makePopupContent(hosts) {
		const makeHostBtn = (host) => {
			const {name, hostid, groupid} = host.properties;
			let popup_data = {type: 'host', data: {hostid: hostid}};
			
			// Inclui o groupid no menu popup quando disponível
			if (groupid !== undefined) {
				popup_data.data.groupid = groupid;
			}
			
			const data_menu_popup = JSON.stringify(popup_data);
			...
			hosts.forEach(host => {
				...
				rows += `
					<tr data-hostid="${host.properties.hostid}" ${row_class}>
						...
						${makeDataCell(host, CWidgetGeoMapMonzphere.SEVERITY_DISASTER)}
						${makeDataCell(host, CWidgetGeoMapMonzphere.SEVERITY_HIGH)}
						${makeDataCell(host, CWidgetGeoMapMonzphere.SEVERITY_AVERAGE)}
						${makeDataCell(host, CWidgetGeoMapMonzphere.SEVERITY_WARNING)}
						${makeDataCell(host, CWidgetGeoMapMonzphere.SEVERITY_INFORMATION)}
						${makeDataCell(host, CWidgetGeoMapMonzphere.SEVERITY_NOT_CLASSIFIED)}
					</tr>`;
			});
		...
		const html = `
			...
			<tr>
				<th>${t('Host')}</th>
				<th>${this._severity_levels.get(CWidgetGeoMapMonzphere.SEVERITY_DISASTER).abbr}</th>
				<th>${this._severity_levels.get(CWidgetGeoMapMonzphere.SEVERITY_HIGH).abbr}</th>
				<th>${this._severity_levels.get(CWidgetGeoMapMonzphere.SEVERITY_AVERAGE).abbr}</th>
				<th>${this._severity_levels.get(CWidgetGeoMapMonzphere.SEVERITY_WARNING).abbr}</th>
				<th>${this._severity_levels.get(CWidgetGeoMapMonzphere.SEVERITY_INFORMATION).abbr}</th>
				<th>${this._severity_levels.get(CWidgetGeoMapMonzphere.SEVERITY_NOT_CLASSIFIED).abbr}</th>
			</th>
			</thead>
	...
	initSeverities(severities) {
		...
		for (let i = CWidgetGeoMapMonzphere.SEVERITY_NO_PROBLEMS; i <= CWidgetGeoMapMonzphere.SEVERITY_DISASTER; i++) {
